...
0p. **Nhập yêu cầu kick-off dạng free text** (11/08, code xong, `tsc --noEmit` exit 0, chưa verify UI) — step "Nhập yêu cầu sơ bộ dự án" trong PhaseWorkflowCard thêm khối **"Hoặc dán yêu cầu dạng văn bản tự do"**: textarea free text + nút "Thêm các yêu cầu này" — tách từng dòng thành 1 yêu cầu (bỏ bullet `-`/`•`/`*` + số thứ tự `1.`/`1)` prefix, bỏ dòng trống; dòng >120 ký tự → title rút gọn `…` + detail giữ nguyên bản đầy đủ). Có đếm dòng "Sẽ tạo N yêu cầu" khi đang gõ. Yêu cầu thêm vào chung danh sách `reqs` → "Lưu & sinh task tracking" hoạt động như cũ (mỗi dòng = 1 task tracking `[Kickoff]`).
