...
export function Column({ date, tasks, totalHours }: ColumnProps) {
  ...
  return (
    <div className={`flex flex-col w-[345px] shrink-0 rounded-2xl border transition-colors duration-300 group ${isOverflow ? 'bg-destructive/10 border-destructive/30 glow-red' : 'glass-panel'} `}>
      <div className={`p-3 py-2.5 border-b flex justify-between items-center rounded-t-2xl relative ${isOverflow ? 'border-destructive/20 bg-destructive/10' : 'border-border/50 bg-muted/20'}`}>
